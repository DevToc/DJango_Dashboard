!(function (s) {
	...
	(e.prototype.init = function () {
		s('.select2').select2(),
			...
			s('#colorpicker-showinput-intial').spectrum({ showInitial: !0, showInput: !0 }),
			s('#timepicker').timepicker({ icons: { up: 'mdi mdi-chevron-up', down: 'mdi mdi-chevron-down' }, appendWidgetTo: '#timepicker-input-group1' }),
			s('#timepicker2').timepicker({ showMeridian: !1, icons: { up: 'mdi mdi-chevron-up', down: 'mdi mdi-chevron-down' }, appendWidgetTo: '#timepicker-input-group2' }),
			s('#timepicker3').timepicker({ minuteStep: 15, icons: { up: 'mdi mdi-chevron-up', down: 'mdi mdi-chevron-down' }, appendWidgetTo: '#timepicker-input-group3' });
		var i = {};
		s('[data-toggle="touchspin"]').each(function (e, t) {
			var a = s.extend({}, i, s(t).data());
			s(t).TouchSpin(a);
		}),
			s("input[name='demo3_21']").TouchSpin({ initval: 40, buttondown_class: 'btn btn-primary', buttonup_class: 'btn btn-primary' }),
			s("input[name='demo3_22']").TouchSpin({ initval: 40, buttondown_class: 'btn btn-primary', buttonup_class: 'btn btn-primary' }),
			s("input[name='demo_vertical']").TouchSpin({ verticalbuttons: !0 }),
			s('input#defaultconfig').maxlength({ warningClass: 'badge bg-info', limitReachedClass: 'badge bg-warning' }),
			s('input#thresholdconfig').maxlength({ threshold: 20, warningClass: 'badge bg-info', limitReachedClass: 'badge bg-warning' }),
			s('input#moreoptions').maxlength({ alwaysShow: !0, warningClass: 'badge bg-success', limitReachedClass: 'badge bg-danger' }),
			...
			s('textarea#textarea').maxlength({ alwaysShow: !0, warningClass: 'badge bg-info', limitReachedClass: 'badge bg-warning' }),
			s('input#placement').maxlength({ alwaysShow: !0, placement: 'top-left', warningClass: 'badge bg-info', limitReachedClass: 'badge bg-warning' });
	}),
