...
def req74right(total_df): #same customer, same faimly, different prices  Values: silicon or package level, different customers.  Y: ASP in EUR, X: year.
    ...
    for index, row in keyDf.iterrows():
        ...
        total_df                                    \
        WHERE                                       \
        price != 0 and gen = '" + str(row['gen']) + "' and series = '" + str(row['series']) + "' and package = '" + str(row['package']) + "'  \
        GROUP BY                                    \
...
def req74left(total_df): #same customer, same faimly, different prices  Values: silicon or package level, different customers.  Y: ASP in EUR, X: year.
    ...
    print("start keydf test", starttime)

    keyDf = ps.sqldf(keyQuery, locals())
    keyDf = keyDf.reset_index()

    endtime = datetime.now()
    print("end timing keydf test", endtime - starttime)
    ...
    for index, row in keyDf.iterrows():
        ...
        ;                                           \
        "
        valueDf = ps.sqldf(valueQuery, locals()) # -> das macht super langsam
        ...
        jsonResult[str(index)]=jsonElement
        print("jsonElement", str(index))
        print(jsonElement)

    #endtime = datetime.now()
    #print("end timing test", endtime - starttime)

    print("data graph 74 left")
    print(jsonResult)
    return jsonResult
...
class KPIDashboard2(APIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [SessionAuthentication]

    def get(self, request):
        print("getting KPI Dashboard2")

        ###
        starttime = datetime.now()
        print("start timint test", starttime)

        total_df = restructure_whole()
        print("total df ", datetime.now())
        right = req74right(total_df)
        print("REQ 74 right ", datetime.now())
        left = req74left(total_df)
        print("REQ 74 left ", datetime.now())
        filterJson = basicFilter()
        endtime = datetime.now()
        print("end timing test", endtime)

        print("end timing test", endtime - starttime)


        print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
        print("getting json",left, right)
        return JsonResponse({"req74_data_right": right, "req74_data_left": left, "filter":filterJson}, safe=True)
