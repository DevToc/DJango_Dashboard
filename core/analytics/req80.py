...
def req80(total_df,this_year): #same customer, same faimly, different prices  Values: silicon or package level, different customers.  Y: ASP in EUR, X: year.
    ...
    for index, row in keyDf.iterrows(): #count /45 because for each project we have 45 entries (45 years)
        ...
        FROM(                                           \
            SELECT                                      \
            series, endCustomer_id, SUM(gm) as CYgm , SUM(wVol) as CYwvol, SUM(fy_gm) as FYgm , SUM(fy_wVol) as FYwvol, SUM(wRev) as CY_wRev, SUM(fy_wRev) as FY_wRev,  5 as start   \
            FROM                                        \
            total_df                                    \
            WHERE                                       \
...
class KPIDashboard8(APIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [SessionAuthentication]

    def get(self, request):
        print("getting KPI Dashboard8")
        ...
        json = req80(total_df,year)
        filterJson = basicFilter()

        print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
        print("getting json",json)
        return JsonResponse({"req80_data": json, "filter":filterJson}, safe=True)
