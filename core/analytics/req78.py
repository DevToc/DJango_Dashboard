...
def req78(total_df,this_year): #same customer, same faimly, different prices  Values: silicon or package level, different customers.  Y: ASP in EUR, X: year.
...
class KPIDashboard6(APIView):
    permission_classes = [IsAuthenticated]
    authentication_classes = [SessionAuthentication]

    def get(self, request):
        print("getting KPI Dashboard6")
        ...
        json = req78(total_df,year)
        filterJson = basicFilter()

        print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
        print("getting json",json)
        return JsonResponse({"req78_data": json, "filter":filterJson}, safe=True)
